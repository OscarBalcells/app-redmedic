...
contract Profile {
	...
	function revokeAccess(bytes32 name, address addr, bytes32 section) public {
        ...
        if(section == "all") relationships[name].permissions[addr].fullAccess = false;
            ...
            for(uint i = relationships[name].permissions[addr].cats.length-1; i >= 0; i--) {
                if(relationships[name].permissions[addr].cats[i] == section) {
                    relationships[name].permissions[addr].cats[i] = relationships[name].permissions[addr].cats[relationships[name].permissions[addr].cats.length-1];
                    relationships[name].permissions[addr].cats.length--;
                }
            }
        }
	}
    ...
    function hasAccess(bytes32 name, address addr, bytes32 section) public view returns (bool) {
        ...
        for(uint i = 0; i < relationships[name].permissions[addr].cats.length; i++) {
            if(section == relationships[name].permissions[addr].cats[i]) {
                return true;
            }
        }
        return false;
    ...
    function returnGateways() public view returns (bytes32[] memory) {
        bytes32[] memory out = new bytes32[](nRelationships);
        for(uint i = 0; i < nRelationships; i++) {
            out[i] = relationships[names[i]].name;
        }
        return out;
    }
    ...
    function getRelationship(bytes32 name) public view returns (address, bytes32) {
        require(relationships[name].exists == true);
        return (relationships[name].addr, relationships[name].gateway);
    }
}

...
contract Acta {
    ...
    */
	...
	event ProposalSaved(bytes32 eventName, address proposer, uint proposalId);
	event ProposalExecuted(bytes32 eventName, uint proposalId);
    ...
    function internalActionProposal(uint proposalId, uint numData, bytes32 bytesData1, bytes32 bytesData2, address addrData) private {
        ...
        nProposals++;
        emit ProposalSaved("ProposalSaved", msg.sender, proposalId);
    }
    ...
    function voteProposal(uint proposalIndex) public {
		require(proposals[proposalIndex].proposalId > 0 && isMember[msg.sender] == true && votes[proposalIndex][msg.sender] == false);
	...
	function undoVoteProposal(uint proposalIndex) public {
        require(proposals[proposalIndex].proposalId > 0 && isMember[msg.sender] == true && votes[proposalIndex][msg.sender] == true);
    ...
    function executeProposal(uint proposalIndex) private {
        ...
        emit ProposalExecuted("ProposalExecuted", proposal.proposalId);
    }
